...
const AddSessionModal: React.FC<AddSessionModalProps> = ({
  ...
  // 额外的会话类型状态
  const [sessionTypeExtra, setSessionTypeExtra] = useState({
    session: '',
    isTournament: false,
  });
  ...
  const validateRequiredFields = () => {
    const errors = {
      session: !sessionTypeExtra.session,
              ...
              <TouchableOpacity
                ...
                <Text
                  style={[
                    ...
                    sessionTypeExtra.session ? styles.selectedText : null,
                    fieldErrors.session && styles.errorText,
                  ]}>
                  {sessionTypeExtra.session || 'Please select'}
                </Text>
