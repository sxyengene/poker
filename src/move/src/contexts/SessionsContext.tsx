...
export const SessionsProvider: React.FC<SessionsProviderProps> = ({
  ...
  const loadSessions = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const loadedSessions = await sessionStorage.getSessions();
  ...
  const addSession = useCallback(
    async (newSession: NewSession): Promise<SessionData> => {
      try {
        setError(null);
        const savedSession = await sessionStorage.saveSession(newSession);
  ...
  const getStats = useCallback(async () => {
    try {
      return await sessionStorage.getStats();
    } catch (error) {
